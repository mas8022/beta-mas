...
const TaskCard = ({ task, setEditId, setValue }) => {
  ...
  const removeTaskFetcher = async (id) => {
    try {
      const { status, message } = await secretFetch.delete(`/api/tasks/${id}`);
      status > 201 && toast.error(message);
    } catch (error) {
  ...
  const toggleDoneFetcher = async ({ id }) => {
    const { status, message } = await secretFetch.patch(`/api/tasks/${id}`);
  ...
  const { mutate: toggleDone } = useMutation({
    ...
    onMutate: async ({ id, done }) => {
      ...
      const previousData = queryClient.getQueryData(["my-tasks"]);
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 mt-2">
        <Button
          size="sm"
          onClick={() => toggleDone({ id: task.id, done: task.done })}
        >
